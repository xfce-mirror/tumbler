...
if test x"$ac_tumbler_pixbuf_thumbnailer" = x"yes"; then
  ...
  PKG_CHECK_MODULES([GDK_PIXBUF], [gdk-pixbuf-2.0 >= 2.42.8], [], [ac_tumbler_pixbuf_thumbnailer=no])
fi
...
if test x"$ac_tumbler_font_thumbnailer" = x"yes"; then
  dnl Check for gdk-pixbuf 
  PKG_CHECK_MODULES([GDK_PIXBUF], [gdk-pixbuf-2.0 >= 2.42.8],
  [
...
if test x"$ac_tumbler_ffmpeg_thumbnailer" = x"yes"; then
  ...
  [
    ...
    PKG_CHECK_MODULES([FFMPEGTHUMBNAILER], [libffmpegthumbnailer >= 2.2.2], [], [ac_tumbler_ffmpeg_thumbnailer=no])
...
if test x"$ac_tumbler_desktop_thumbnailer" = x"yes"; then
  dnl Check for gdk-pixbuf
  PKG_CHECK_MODULES([GDK_PIXBUF], [gdk-pixbuf-2.0 >= 2.42.8], [], [ac_tumbler_desktop_thumbnailer=no])
fi
