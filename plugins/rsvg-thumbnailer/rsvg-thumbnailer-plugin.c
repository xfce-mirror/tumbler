...
#ifdef DEBUG
  g_print ("Initializing the Tumbler Rsvg Thumbnailer plugin\n");
#endif

  /* register the types provided by this plugin */
  rsvg_thumbnailer_register (plugin);
  rsvg_thumbnailer_provider_register (plugin);

  /* set up the plugin provider type list */
  type_list[0] = TYPE_RSVG_THUMBNAILER_PROVIDER;
}
...
{
#ifdef DEBUG
  g_print ("Shutting down the Tumbler Rsvg Thumbnailer plugin\n");
#endif
}
